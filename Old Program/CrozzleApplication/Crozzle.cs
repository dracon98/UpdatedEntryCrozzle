...
namespace CrozzleApplication
{
        ...
        Crozzle
    {
        ...
        public List<String> CrozzleGridErrors { get; set; }
        ...
        public List<String[]> CrozzleRows { get; set; }
        public List<String[]> CrozzleColumns { get; set; }
        ...
        private int CrozzleScore()
        {
            int score = 0;
            // New UPDATE
            // New loop for score listed in the sequence
            // list all the sequence in the crozzle
            foreach(WordData Sequence in CrozzleSequences.Sequences)
            {
                // loops in all wordlist
                foreach (String Point in Configuration.PointsPerWord)
                {
                    string[] PointPerWordRow = Point.Split(',');
                    // for match 
                    if (Regex.IsMatch(PointPerWordRow[0], @"^" + Sequence.Letters + "$"))
                    {
                        int point;
                        if (Validator.IsInt32(PointPerWordRow[1], out point))
                            score += point;
                    }
                }
            }
        ...
        public void LogFileErrors(String errors)
        {
            ...
            {
                StreamWriter logfile = new StreamWriter(CrozzleDirectoryName + @"\log.txt", true);
                Errors.Add(String.Format(ConfigurationErrors.LogFileError));
                logfile.WriteLine(errors);
                logfile.Close();
            }
